...
**Prerequisite:** Phase 4 complete — the full loop runs: candidates → deliberation →
risk gate → order → position management → exit → lesson, all visible in the dashboard.
...
ideally two**, and there are only a couple left before the deadline.
...
- [ ] `risk.py` imports nothing from `crew.py`, `llm.py`, or `trading/strategist.py`.
...
4. Tune based on what you see, not on what you expected:
   - Zero trades in a session almost always means the risk gates or liquidity filters are too
     tight, or the PM is too conservative. Loosen the *thresholds*, never the safety rules.
...
- A copy of the most interesting decision chain (evidence → three arguments → verdict →
