"""Replay the options-m agent over 24-28 August 2026 against the *v2* exit rules.
...
Read the difference against that run's ``report.md``: same entries, same fills,
only the close signals move.
...
ROOT = HERE.parents[2]
# The bars, contracts and calendar of the run this one is compared against.
# Same bytes, same fingerprint -- copying 13 MB to change an exit rule would
# only make it harder to prove the data did not move.
DATA = HERE.parent / "2026-08-30_universe_exit-flow_1Day" / "raw"
...
def position_payload(position: Position, mark: float, day: date) -> dict[str, Any]:
    """The cache row ``PositionManagerAgent`` would have written for this mark.

    The whole input surface of the exit rule, and nothing else: the net the
    P&L is measured against, the days left on the nearest leg, the strategy
    that picks the thresholds, and the fill date the calendar stop counts
    from.

    ``mid_value`` is already signed per contract (a credit structure marks
    negative), so the net and the gross differ only in that the gross drops
    the sign -- which is exactly the distinction the shipped agent draws.
    """
    scale = position.qty * CONTRACT_MULTIPLIER
    net_value = mark * scale
    unrealized_pl = (mark - position.entry_value) * scale
    payload: dict[str, Any] = {
        "market_value": abs(net_value),
        "net_value": net_value,
        "unrealized_pl": unrealized_pl,
        "min_dte": min_dte(position, day),
...
def min_dte(position: Position, day: date) -> int:
    """Days from ``day`` to the nearest leg expiry."""
    return min((date.fromisoformat(leg["expiry"]) - day).days for leg in position.legs)
    ...
    *,
    ...
    for index, day in enumerate(WEEK):
        ...
        for symbol in UNIVERSE:
            ...
            with frozen_at(day):
                mcp = AsOfMcp(DATA, as_of=day, spread_pct=spread_pct)
                try:
    ...
    return {"day_logs": day_logs, "positions": positions, "settings": {
        ...
        "exit_dte_hard_floor": settings.exit_dte_hard_floor,
        "exit_dte_short_premium": settings.exit_dte_short_premium,
        "exit_credit": [settings.exit_credit_profit_target_pct,
                        settings.exit_credit_stop_loss_pct],
        "exit_debit": [settings.exit_debit_profit_target_pct,
                       settings.exit_debit_stop_loss_pct],
        "exit_long": [settings.exit_long_profit_target_pct,
                      settings.exit_long_stop_loss_pct],
    }}
...
def main() -> None:
    ...
    cfg = result["settings"]
    rule = "=" * 92
    print(f"\n{rule}\nEXIT-RULES-V2 REPLAY 24-28 Aug 2026 | spread={args.spread_pct:.1%} "
          f"max_concurrent={cfg['max_concurrent_positions']} "
          f"exits={'on' if cfg['exits_enabled'] else 'OFF'}\n"
          f"dte: hard={cfg['exit_dte_hard_floor']}d short-premium={cfg['exit_dte_short_premium']}d"
          f" | credit +{cfg['exit_credit'][0]:.0%}/-{cfg['exit_credit'][1]:.0%}"
          f" | debit +{cfg['exit_debit'][0]:.0%}/-{cfg['exit_debit'][1]:.0%}"
          f" | long +{cfg['exit_long'][0]:.0%}/-{cfg['exit_long'][1]:.0%}"
          f" | time {cfg['exit_time_stop_days']}d\n{rule}")
